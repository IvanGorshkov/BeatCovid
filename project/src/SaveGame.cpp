...
std::string Save::GetLvlName() {
  ...
  std::string lvlname = FILES_PATH"files/maps/";
...
bool Save::SaveExists() {
  std::fstream save;
  save.open(FILES_PATH"files/saves/save.txt");
  return save.is_open();
}

bool Save::SaveExistsP() {
  std::fstream save;
  save.open(FILES_PATH"files/saves/save_points.txt");
  return save.is_open();
}

bool Save::SaveExistsA() {
  std::fstream save;
  save.open(FILES_PATH"files/saves/save_armor.txt");
  return save.is_open();
}
...
void Save::SaveGame(GameManager &game) const {
  std::ofstream save_file(FILES_PATH"files/saves/save.txt");
  // lvl
  save_file << this->lvl;
  save_file << std::endl;
  // arm
...
std::vector<int> Save::LoadArmors() {
  std::fstream saveArmorsFile(FILES_PATH"files/saves/save_armor.txt");
...
void Save::SaveArmor(std::vector<int> armors) {
  std::ofstream saveArmorsFile(FILES_PATH"files/saves/save_armor.txt");
...
int Save::LoadPoints() {
  std::fstream save(FILES_PATH"files/saves/save_points.txt");
  if (!save.is_open()) {
    return 0;
  }
...
void Save::SavePoints(int points) {
  std::ofstream save_file(FILES_PATH"files/saves/save_points.txt");
  save_file << points;
  save_file.close();
}
...
std::vector<float> Save::LoadConfig() {
  std::fstream saveConfigFile(FILES_PATH"files/saves/save_config.txt");
